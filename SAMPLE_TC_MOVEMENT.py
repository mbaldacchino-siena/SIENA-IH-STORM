...
def TC_movement(lon_genesis_list,lat_genesis_list,basin,phase=None):
    ...
    for lat_genesis,lon_genesis in zip(lat_genesis_list,lon_genesis_list):
        ...
        landfalllijst=[Check_if_landfall(lat_genesis,lon_genesis,lat1,lon0,land_mask)]
        lat=lat_genesis
        lon=lon_genesis
        var=0
        while var==0:
            ...
            row = constants[ind]
            if len(row) >= 17:
                [a_lat0,a_lat1,a_lat2,g_en,g_ln,b_lon0,b_lon1,d_en,d_ln,Elatmu,Elatstd,Elonmu,Elonstd,Dlat0mu,Dlat0std,Dlon0mu,Dlon0std]=row
            else:
                [b_lon0,b_lon1,a_lat0,a_lat1,a_lat2,Elatmu,Elatstd,Elonmu,Elonstd,Dlat0mu,Dlat0std,Dlon0mu,Dlon0std]=row
            ...
            if basin in ('SP','SI'):
                dlat1=float(dlat1 + (-abs(np.random.normal(Elatmu,Elatstd)) if lat>-10 else np.random.normal(Elatmu,Elatstd)))
            else:
                dlat1=float(dlat1 + (abs(np.random.normal(Elatmu,Elatstd)) if lat<10 else np.random.normal(Elatmu,Elatstd)))
            dlon1=float(b_lon0 + b_lon1*dlon0 + phase_lon + np.random.normal(Elonmu,Elonstd))
            if np.abs(lat)>=45 and dlon1<0.:
                dlon1=0
