...
def sample_rmax(p, rmax_pres):
    if p > 940.0:
        r = np.random.choice(rmax_pres[2])
    elif p <= 940.0 and p > 920.0:
        r = np.random.choice(rmax_pres[1])
    else:
        r = np.random.choice(rmax_pres[0])
    return float(r)
...
def Add_Rmax(pressure):
    ...
    if (
        rmin < rgenesis and ind > 0
    ):  # if the new radius is smaller than the old one, AND the min pressure is not at genesis
    ...
    if rdis > rind:  # if radius is larger than the last added radius to the rmax_list:
        for i in range(ind + 1, len(pressure)):
            radius = (
                (rdis - rind) / (len(pressure) - 1 - ind) * i
                + rdis
                - (len(pressure) - 1) * (rdis - rind) / (len(pressure) - 1 - ind)
            )
            rmaxlist.append(radius)
    else:
        for i in range(ind + 1, len(pressure)):
