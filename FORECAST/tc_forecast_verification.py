...
def compare_seasons(
    ...
    *,
    ...
    """Compare synthetic ensemble vs observed (IBTrACS) vs CSU April forecasts.

    Each forecast year `Y` has K members (typically 51); each member is one
    TCTracks containing N simulated seasons of Y. Seasons are pooled across
    all K * N samples; mean, 5th/50th/95th percentiles and landfall
    ...
    expected_sim_years_per_member :
        ...
        the pooled distribution. STRONGLY RECOMMENDED. If left as None, a
        member that happens to have no storms in some sim_year will silently
        produce no row for that season, biasing the per-season averages
        UPWARD. See `process_year_pooled` for details.

    Returns
    -------
    df : DataFrame
        MultiIndex (year, source). Sources include 'observed', 'csu_april',
        and 'synthetic' (the pooled mean + percentile rows).
    seasons_by_year (only if return_seasons=True) :
        dict[year -> per-(member, sim_year) DataFrame] for ad-hoc analysis,
        e.g. plotting full empirical distributions per year.
    """
    if years is None:
...
def skill_summary(
    ...
    forecast_sources: Sequence[str] = ("csu_april", "synthetic_mean"),
...
# Plotting
# ---------------------------------------------------------------------------
def plot_metric_comparison(df: pd.DataFrame, metric: str, ax=None):
...
def plot_distribution_per_year(
    ...
    Optionally overlay observed and CSU values from `df_summary`.
    """
    import matplotlib.pyplot as plt
